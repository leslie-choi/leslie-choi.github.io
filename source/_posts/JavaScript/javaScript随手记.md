...
null：Null 类型，代表“空值”，代表一个**空对象指针**，使用 typeof 运算得到 **“object”**，所以可以认为它是一个特殊的对象值。

undefined：Undefined 类型，声明但是未初始化变量时，得到的就是 undefined。
...
也可以认为 undefined 是表示系统级的、出乎意料的或类似错误的值的空缺，而 null 是表示程序级的、正常的或在意料之中的值的空缺。